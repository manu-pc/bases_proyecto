...
SELECT mp.id_material AS id_material,
...
WHERE mp.fecha_prestamo IS NOT NULL;
...
WHERE mp.fecha_prestamo IS NULL;
...
SELECT e.dni AS dni_empleado,
    e.nombre AS nombre_empleado,
    c.cargo
FROM empleado e
    JOIN cargo c ON e.dni = c.dni_empleado
WHERE c.cargo = 'bibliotecario';
...
SELECT o.id_ordenador AS ID,
    o.SO AS Sistema_Operativo,
    o.modelo AS modelo
FROM ordenador o
WHERE o.usuario IS NULL;
